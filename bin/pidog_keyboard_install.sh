...
# ========================
if [ $(id -u) -ne 0 ]; then
  printf "Script must be run as root. Try 'sudo bash pidog_keyboard_install.sh'\n"
  exit 1
fi
...
echo -e "${GREEN}install pidog_keyboard interactive commands ...${NC}"
cp ./pidog_keyboard /usr/local/bin/
chmod +x /usr/local/bin/pidog_keyboard

# ==================================== auto-start ====================================
echo -e "${GREEN}install pidog_keyboard auto-start service ...${NC}"
# https://www.raspberrypi.com/documentation/computers/configuration.html#software-install

# https://www.freedesktop.org/software/systemd/man/systemd.service.html

cat > /usr/lib/systemd/system/pidog_keyboard.service << EOF
[Unit]
Description=pidog_keyboard service
...
systemctl enable pidog_keyboard.service
systemctl restart pidog_keyboard.service
# sudo systemctl status pidog_keyboard.service
# journalctl -u pidog_keyboard.service
# journalctl -u pidog_keyboard.service --lines=50
